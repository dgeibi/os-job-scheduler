...
class OptsForm extends Component {
  ...
  validMemSize = (rule, value, callback) => {
    ...
    if (Number.isNaN(x) || Number.isNaN(n) || n < x) {
      return callback()
    }
    return callback('作业内存大小必须小于总内存')
  }
      ...
      <Row gutter={24} key={k}>
        ...
        <Col span={8}>
          <FormItem required={false}>
            ...
            })(<Input placeholder="正数" {...minusCircle(k)} />)}
          </FormItem>
        </Col>
      </Row>
    ))
    ...
    return (
      <Form onSubmit={this.handleSearch}>
        ...
        <Row gutter={24}>
          ...
          <Col span={8}>作业需要的时间：</Col>
          <Col span={8}>作业内存大小：</Col>
        </Row>
